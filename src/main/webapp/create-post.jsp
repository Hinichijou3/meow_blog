...
<body>
    <div class="container">
        ...
        <header class="page-header">
            ...
            <p class="page-subtitle">开始您的创作之旅</p>
        </header>
        ...
        <div class="edit-container">
            <div class="form-header">
                ...
                <div>
                    <span style="color: #666;">开始新的创作</span>
                </div>
            </div>
            ...
            <div class="writing-tips">
                ...
                <ul>
                    <li>使用清晰的标题吸引读者</li>
                    <li>摘要可以帮助读者快速了解文章内容</li>
                    <li>使用工具栏格式化文本，支持Markdown语法</li>
                    <li>可以先保存为草稿，稍后发布</li>
                </ul>
            </div>
            ...
            <form id="postForm" action="create-post" method="post">
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" class="form-control" 
                           placeholder="请输入文章标题（建议50字以内）" required maxlength="100">
                ...
                <div class="form-group">
                    ...
                    <textarea id="excerpt" name="excerpt" class="form-control" 
                              placeholder="请输入文章摘要（可选，建议150字以内）" maxlength="300"></textarea>
                    <div class="char-count">
                ...
                <div class="form-group">
                    ...
                    <div class="editor-toolbar">
                        <button type="button" class="toolbar-btn" onclick="formatText('bold')" title="加粗"><strong>B</strong></button>
                        <button type="button" class="toolbar-btn" onclick="formatText('italic')" title="斜体"><em>I</em></button>
                        <button type="button" class="toolbar-btn" onclick="formatText('underline')" title="下划线"><u>U</u></button>
                        ...
                        <button type="button" class="toolbar-btn" onclick="insertText('### ')" title="三级标题">H3</button>
                        <button type="button" class="toolbar-btn" onclick="insertText('- ')" title="无序列表">列表</button>
                        <button type="button" class="toolbar-btn" onclick="insertText('1. ')" title="有序列表">1.列表</button>
                        <button type="button" class="toolbar-btn" onclick="insertText('> ')" title="引用">引用</button>
                        <button type="button" class="toolbar-btn" onclick="insertText('`代码`')" title="行内代码">代码</button>
                        <button type="button" class="toolbar-btn" onclick="insertText('[链接文字](URL)')" title="链接">链接</button>
                        <button type="button" class="toolbar-btn" onclick="togglePreview()" title="预览">预览</button>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 状态选择变化时更新按钮文字
            document.getElementById('status').addEventListener('change', updateCharCount);
        });
        ...
        function insertText(text) {
            ...
            // 如果是列表或引用，在选中文本的每一行前添加
            if (text === '- ' || text === '1. ' || text === '> ') {
                if (selectedText) {
                    const lines = selectedText.split('\n');
                    newText = lines.map(line => text + line).join('\n');
        ...
        function togglePreview() {
            ...
            if (previewArea.style.display === 'block') {
                ...
                previewArea.innerHTML = html || '<p style="color: #999; text-align: center;">输入内容后预览将显示在这里</p>';
        ...
        function autoSaveDraft() {
            ...
            autoSaveTimer = setTimeout(function() {
                if (formChanged) {
                    console.log('自动保存草稿...');
                    // 这里可以实现自动保存到localStorage或发送AJAX请求
                }
            }, 5000); // 5秒后自动保存
        }
        
        // 添加自动保存监听
        document.getElementById('contentEditor').addEventListener('input', autoSaveDraft);
